...
#include "stm32l4.h"
#include "chip/stm32l4_dsi.h"
#include "stm32l4_ltdc.h"
#include "stm32l4_dsi.h"

static DSI_HandleTypeDef hdsi1;

static uint32_t HAL_GetTick(){return 0;}

/** @addtogroup STM32L4xx_HAL_Driver
  * @{
  */
#define HAL_DSI_MODULE_ENABLED
#define DSI	((DSI_TypeDef *)STM32L4_DSI_BASE)
...
int stm32l4_dsiinitialize(void)
{
	...
	hdsi1.Instance = DSI;
	hdsi1.Init.AutomaticClockLaneControl = DSI_AUTO_CLK_LANE_CTRL_DISABLE;
	hdsi1.Init.TXEscapeCkdiv = 2;
	hdsi1.Init.NumberOfLanes = DSI_ONE_DATA_LANE;
	PLLInit.PLLNDIV = 10;
	PLLInit.PLLIDF = DSI_PLL_IN_DIV1;
	PLLInit.PLLODF = DSI_PLL_OUT_DIV1;

	if (HAL_DSI_Init(&hdsi1, &PLLInit) != HAL_OK)
	{
		gdbg("HAL_DSI_Init error");
	}
	...
	if (HAL_DSI_ConfigHostTimeouts(&hdsi1, &HostTimeouts) != HAL_OK)
	{
		gdbg("HAL_DSI_ConfigHostTimeouts error");
	}
	...
	if (HAL_DSI_ConfigPhyTimer(&hdsi1, &PhyTimings) != HAL_OK)
	{
		gdbg("HAL_DSI_ConfigPhyTimer error");
	}
	if (HAL_DSI_ConfigFlowControl(&hdsi1, DSI_FLOW_CONTROL_BTA) != HAL_OK)
	{
		gdbg("HAL_DSI_ConfigFlowControl error");
	}
	if (HAL_DSI_SetLowPowerRXFilter(&hdsi1, 10000) != HAL_OK)
	{
		gdbg("HAL_DSI_SetLowPowerRXFilter error");
	}
	if (HAL_DSI_ConfigErrorMonitor(&hdsi1, HAL_DSI_ERROR_NONE) != HAL_OK)
	{
		gdbg("HAL_DSI_ConfigErrorMonitor error");
	}
	...
	if (HAL_DSI_ConfigCommand(&hdsi1, &LPCmd) != HAL_OK)
	{
		gdbg("HAL_DSI_ConfigCommand error");
	}
	return 0;
}
